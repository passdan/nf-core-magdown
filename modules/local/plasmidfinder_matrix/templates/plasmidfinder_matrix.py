...
Cells contain "identity% / coverage%" for the best hit (highest identity)
of each plasmid in each sample. Coverage is derived from PlasmidFinder's
"Query / Template length" column. Plasmids absent from a sample are blank.

This is a Nextflow template: only ${variable} is interpolated by Nextflow;
backslashes are literal, so normal Python escapes (\\t, \\d) work as written.
"""
...
frames = []
for path in sorted(glob.glob("*.plasmidfinder_summary.tsv")):
    try:
...
if not frames or all(f.empty for f in frames):
    ...
    pd.DataFrame(columns=["sample"]).to_csv(f"{PREFIX}.plasmidfinder_matrix.tsv", sep="\t", index=False)
    pd.DataFrame(columns=long_cols).to_csv(f"{PREFIX}.plasmidfinder_long.tsv", sep="\t", index=False)
else:
    ...
    long.to_csv(f"{PREFIX}.plasmidfinder_long.tsv", sep="\t", index=False)
    ...
    matrix.to_csv(f"{PREFIX}.plasmidfinder_matrix.tsv", sep="\t")
